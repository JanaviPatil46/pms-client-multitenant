...
const FolderTreeView = ({ accountId }) => {
  ...
  const handleContextMenu = (e, item) => {
    ...
    // Don't show context menu for restricted items
    const restrictedFolderName = "firm docs shared with client";
    const isRestricted = item.type === 'folder' && 
      item.name?.trim().toLowerCase() === restrictedFolderName.trim().toLowerCase();
  ...
  const handleContextMenuAction = (action) => {
    ...
    // Set selected item for operations
    setSelectedFolderForMenu(item);

    switch (action) {
      case 'open':
        if (isFolder) {
          toggleFolder(item.path, isLocked);
        } else if (isFile) {
          handleFileClick(item.path, item.name, item.meta);
        }
        break;
  ...
  const getContextMenuItems = (item) => {
    ...
    const isLocked = item.meta?.readOnly === true;
    const isRead = item.meta?.readStatus === true;
    const isParent = !item.path.includes('/') && isFolder;
    
    // Parent folder (root level) - limited options
    if (isParent) {
      return [
        {
          name: 'New Folder',
    ...
    if (isFile) {
      ...
      // Add share option if not locked
      if (!isLocked) {
        items.splice(3, 0, {
          name: 'Share',
          icon: ShareIcon,
