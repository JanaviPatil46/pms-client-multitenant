...
const attachInterceptors = (api) => {
  api.interceptors.request.use(
    (config) => {
      ...
      const tenantId = sessionStorage.getItem("tenantId"); // optional
      ...
      if (tenantId) {
        config.headers["x-tenant-id"] = tenantId; // optional
      }
...
templateApi.interceptors.request.use((config) => {
  const token = sessionStorage.getItem("token"); // JWT stored in localStorage
  if (token) {
    config.headers["Authorization"] = `Bearer ${token}`;
  }
  return config;
...
export const sidebarAPI = {
  getSidebar: () => sidebarApi.get("/clientsidebar/"),
};
...
export const contactsAPI = {
  ...
  ContCtForgotPassword: (data) =>
    accountcontactApi.post("/api/contactauth/forgot-password", data),

  ContCtVerifyResetToken: (token) =>
    accountcontactApi.get(`/api/contactauth/verify-reset-token/${token}`),

  ContCtResetPassword: (token, data) =>
    accountcontactApi.post(`/api/contactauth/reset-password/${token}`, data),
  ContCtLogin: (data) =>
  accountcontactApi.post("/api/contactauth/login", data),
};
